...
export const ROLE_DEFS = {
  ...
  villager: {
    ...
    popupBullets: ['No night action', 'Wins when all Werewolves are eliminated'],
    usesPerGame: 0
  },
  ...
  seer: {
    ...
    blurb: 'Once per game: choose exactly 4 players (dead or alive). Learn how many of them are evil — not which ones.',
    popupBullets: [
  ...
  sheriff: {
    ...
    blurb: 'Once per game: shoot one player at night. A Werewolf-team target dies. Anyone else and you die instead.',
    popupBullets: [
  ...
  tracker: {
    ...
    blurb: 'Once per game: choose one living player. Learn whether they took any night action, or stayed home.',
    popupBullets: [
  ...
  doctor: {
    ...
    blurb: 'Every night: protect one player from death. Can protect yourself. Can repeat the same target.',
    popupBullets: [
  ...
  veteran: {
    ...
    blurb: 'Once per game: go on Alert. Anyone who visits you that night dies, unless the Doctor protects them.',
    popupBullets: [
  ...
  mayor: {
    ...
    blurb: 'Passive: your vote always counts as 2, including in a revote. Your identity is never revealed by normal means.',
    popupBullets: [
  ...
  werewolf: {
    ...
    blurb: 'Each night, vote with your fellow Werewolves to choose one player to eliminate. Cannot target a teammate.',
    popupBullets: [
  ...
  mageWerewolf: {
    ...
    blurb: 'Once per game: silence one player. Their vote and night action fail — effective the following day and night, not immediately.',
    popupBullets: [
    ...
    optional: true,
    usesPerGame: 1,
    night: { type: 'silence', allowSelf: true, prompt: 'Choose who to silence (effective the following day and night)' }
  },
  ...
  poisoner: {
    ...
    blurb: 'Once per game: poison a player. Announced next morning, they die the following night unless the Doctor cures them.',
    popupBullets: [
  ...
  jester: {
    ...
    blurb: 'No ability. Win alone — but only by being voted out during the day.',
    popupBullets: [
  ...
  amnesiac: {
    ...
    blurb: 'Start with no role or team. Once per game, after at least one death, secretly adopt a dead player\'s role without knowing it first.',
    popupBullets: [
    ...
    usesPerGame: 1,
    silenceable: true,
    night: { type: 'adopt', prompt: 'Choose a dead player to secretly adopt their role' }
  },
  ...
  pirate: {
    ...
    blurb: 'Once per game: secretly nominate a duel target. Two nights later, a coin toss decides who dies.',
    popupBullets: [
    ...
    night: { type: 'duel', prompt: 'Choose your duel target (resolves two nights from now)' }
  }
};
...
export function validateSettings(settings, playerCount) {
  ...
  if (playerCount < 4) return { ok: false, error: 'You need at least 4 players to start.', warnings };
  if (settings.werewolfCount < 1) return { ok: false, error: 'You need at least 1 Werewolf.', warnings };

  if (settings.optionalRoles?.seer && playerCount < SEER_MIN_LOBBY_SIZE) {
    warnings.push(`Seer requires at least ${SEER_MIN_LOBBY_SIZE} players — it has been turned off.`);
  }
  if (totalSpecial >= playerCount) {
    return { ok: false, error: 'Too many special roles — leave room for at least 1 Villager.', warnings };
  }
  if (totalWerewolves >= Math.ceil(playerCount / 2)) {
    return { ok: false, error: 'Too many Werewolves for this player count.', warnings };
  }
...
const AVATAR_COLOURS = [
  '#6b2420', '#2f4a28', '#93a6c4', '#8a6d3b', '#5b4b8a',
  '#3b6b6b', '#a35d3b', '#4a4a6b', '#6b8a3b', '#8a3b6b'
];

export function assignAvatarsAndColours(playerIds) {
  const shuffledAvatars = shuffle(Array.from({ length: 10 }, (_, n) => `img/avatars/player${n + 1}.png`));
...
export function checkWinCondition(playersObj) {
  ...
  // Wolves are gone. Poisoner's survival win:
  //   - final 2 (literal), OR
  //   - sole survivor (3.20b #3: solo survival is a superset, still a win).
  if (alive.length <= 2) {
...
export function winnerDisplay(result) {
  ...
  switch (result.primary) {
    case 'jester':
      return { label: 'Jester wins', team: 'neutral', sub: 'The Jester wanted to be voted out — and got their wish.' };
    case 'werewolf':
      return { label: 'Werewolves win', team: 'werewolf', sub: 'The Werewolves now have control of the town.' };
    case 'poisoner':
      return { label: 'Poisoner wins', team: 'neutral', sub: 'The Poisoner survived to the very end.' };
    case 'town': {
      const alongside = result.winners.includes('pirate') ? ' The Pirate survived alongside them.' : '';
      return { label: 'Villagers win', team: 'town', sub: `Every Werewolf has been hunted down.${alongside}` };
    }
    default:
